...
import kotlinx.android.synthetic.main.scanview.*
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.scanBtn).setOnClickListener { btnScan ->
            (btnScan as Button).text = "Scanning...."
            var found = 0
            while (true) {
                val e = list.children.elementAtOrNull(found) ?: break
                if (mConnections.any { con ->
                        con.first.device.address ==
                                e.findViewById<TextView>(R.id.deviceAddress)?.text
                    })
                    found++
                else
                ...
                run {
                    CoroutineScope(Dispatchers.IO).async {
                        withContext(Dispatchers.Main) {
                            btnScan.text = "Rescan LE Device"

                            if (e == null) {
                                mScanResults?.forEach {
                                    var scanresults = mutableListOf<Int>()
                                    val cell: LinearLayout =
                                        layoutInflater.inflate(R.layout.scanview, null)
                                                as LinearLayout
                                    var enabled = true
                                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                        enabled = it.value.isConnectable
                                    }
                                    val name = cell.findViewById<TextView>(R.id.deviceName)
                                    if (it.value.device.name != null && it.value.device.name.isNotEmpty())
                                        name.text = it.value.device.name
                                    else name.text = "Unknown Device"
                                    val address = cell.findViewById<TextView>(R.id.deviceAddress)
                                    address.text = it.key
                                    val rssi = cell.findViewById<TextView>(R.id.signalScan)
                                    rssi.text = it.value.rssi.toString() + "dBm"
                                    val connect = cell.findViewById<Button>(R.id.connectDevice)
                                    val data = cell.findViewById<TextView>(R.id.dataScan)
                                    val obj = object : BleWrapper.BleCallback {
                                        override fun onDeviceReady(
                                            gatt: BluetoothGatt,
                                            bleWrapper: BleWrapper
                                        ) {
                                            mConnections.add(Pair(gatt, bleWrapper))
                                            ...
                                            for (gattService in gatt.services) {
                                                if (gattService.uuid == BleWrapper.HEART_RATE_SERVICE_UUID) {
                                                    ...
                                                    bleWrapper.getNotifications(
                                                        gatt, gattService.uuid,
                                                        gattService.getCharacteristic
                                                            (BleWrapper.HEART_RATE_MEASUREMENT_CHAR_UUID).uuid
                                                    )
    ...
    private fun startScan(onFinish: (String?) -> Unit) {
        if (hasPermissions()) {
            ...
            onFinish("Please enable your bluetooth")
    }
